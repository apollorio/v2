...
function apollo_users_handle_virtual_pages( $wp ) {
	...
	if ( preg_match( '#^id/([a-zA-Z0-9_.-]+)/?$#', $path, $matches ) ) {
		...
		if ( $user && $user->ID > 0 ) {
			...
			// Ensure global functions are available for templates
			if ( ! function_exists( 'apollo_get_user_avatar_url' ) ) {
				// Define in global namespace
				$global_code = '
				function apollo_get_user_avatar_url( $user_id, $size = "thumb" ) {
				...
				function apollo_get_user_cover_url( $user_id ) {
				...
				function apollo_get_profile_url( $user_id = null ) {
					if ( null === $user_id ) {
