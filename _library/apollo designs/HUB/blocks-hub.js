/**
 * ══════════════════════════════════════════════════════════════
 * BLOCKS-HUB.JS — Global icon helper (mandatory, load BEFORE hub.js)
 * ★★★ BRUTAL ICON SYSTEM: RemixIcon ONLY, zero CDN dependency.
 * RemixIcon is loaded in <head> via jsDelivr — always available, instant.
 * ══════════════════════════════════════════════════════════════
 */
(function() {
  'use strict';
  ...
  /**
   * window.iconHTML — BRUTAL: always RemixIcon, zero external CDN.
   * @param {string} name — Apollo icon slug (e.g. 'instagram-s', 'fire-s')
   * @param {string} size — CSS size string (e.g. '22px')
   * @returns {string} HTML string with <i> RemixIcon element
   */
  window.iconHTML = function(name, size) {
    var n = String(name || 'link').replace(/\.svg$/, '');
    var sz = size || '22px';
    var numSz = parseInt(sz, 10) || 22;
    var ri = RI_MAP[n] || 'ri-apps-2-line';
  ...
  console.log('[HUB] blocks-hub.js icon system: BRUTAL RemixIcon mode — zero CDN dependency');
})();
...
/* ── 7. VIDEO (YouTube Autoplay) ── */
window.BLOCKS.video = {
  type: 'video', label: 'Vídeo', icon: 'ri-youtube-line', group: 'media',
  defaults: () => ({ url: '', title: 'Vídeo', autoplay: true, muted: true }),
  preview: (b) => {
    // Extract YouTube video ID from various URL formats
    const ytId = _extractYouTubeId(b.url);
    if (!ytId) {
      return `
        <div class="pv-block pv-video-autoplay" style="display:flex;align-items:center;justify-content:center;min-height:120px;background:#18181b;">
          <div style="text-align:center;opacity:0.4;">
            <i class="ri-youtube-line" style="font-size:32px;display:block;margin-bottom:4px;" aria-hidden="true"></i>
            <span style="font-size:10px;">Cole a URL do YouTube</span>
          </div>
        </div>`;
    }
    // Autoplay muted, scale 1.2 (127%x127%) to hide YouTube controls behind overflow:hidden
    const params = 'autoplay=1&mute=1&controls=0&loop=1&playlist=' + ytId + '&modestbranding=1&rel=0&showinfo=0&iv_load_policy=3';
    return `
      <div class="pv-block pv-video-autoplay">
        <iframe
  ...
  editor: (b, i) => `
    ${_input(i, 'url', b.url, 'https://youtube.com/watch?v=...', 'URL do YouTube')}
    <span class="sb-help">Cole a URL do YouTube. O vídeo toca automaticamente em mute sem controles visíveis.</span>
    ${_input(i, 'title', b.title, 'Título do vídeo', 'Título')}`
};

/** Extract YouTube video ID from URL */
function _extractYouTubeId(url) {
  if (!url) return null;
  var m = url.match(/(?:youtu\.be\/|youtube\.com\/(?:watch\?v=|embed\/|shorts\/|v\/))([a-zA-Z0-9_-]{11})/);
  return m ? m[1] : null;
}
...
window.BLOCKS.text = {
  ...
  preview: (b) => {
    ...
    var iconH = b.icon ? '<div style="text-align:' + align + ';margin-bottom:8px;font-size:24px;">' + window.iconHTML(b.icon, '24px') + '</div>' : '';
    var tagsHTML = '';
    ...
    return '<div class="pv-block pv-text" style="text-align:' + align + ';">' + iconH + '<p>' + content + '</p>' + statusHTML + '</div>' + tagsHTML;
  },
  editor: (b, i) => `
