...
class APRIO_REST_Authentication extends APRIO_REST_CRUD_Controller
{
    ...
    public function register_routes()
    {
        register_rest_route(
            ...
            [
                [
                    ...
                    'callback'            => [ $this, 'perform_user_authentication' ],
        ...
        register_rest_route(
            ...
            [
                [
                    'methods'             => WP_REST_Server::CREATABLE,
                    'callback'            => [ $this, 'perform_login_authentication' ],
                    'permission_callback' => '__return_true',
                    'args'                => [
                        'username' => [
                            'required'          => true,
    ...
    public function perform_user_authentication($request)
    {
        ...
        if (! empty($username) && ! empty($password)) {
            ...
            if (is_wp_error($user)) {
                ...
                $all_mobile_pages = [ 'dashboard', 'attendees', 'guest_list', 'orders', 'arrivals' ];
                ...
                if ($is_compatibilidade && $enable_compatibilidade) {
                    ...
                    $compatibilidade_details = [
                        ...
                        'skills'                     => isset($user_meta['_skills'][0]) ? $user_meta['_skills'][0] : [],
                        'interests'                  => isset($user_meta['_interests'][0]) ? $user_meta['_interests'][0] : [],
    ...
    public function check_authentication($request)
    {
        ...
        return new WP_Error('rest_forbidden', __('Missing or invalid authorization token.', 'textdomain'), [ 'status' => 401 ]);
    }
    /**
    ...
    private function validate_jwt_token($token)
    {
        $parts = explode('.', $token);
        if (count($parts) !== 3) {
            return new WP_Error('rest_forbidden', __('Malformed token.', 'textdomain'), [ 'status' => 401 ]);
        }
