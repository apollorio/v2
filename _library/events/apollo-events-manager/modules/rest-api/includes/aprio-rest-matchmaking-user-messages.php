...
class APRIO_REST_Send_Message_Controller extends APRIO_REST_CRUD_Controller
{
    ...
    public function aprio_send_compatibilidade_messages($request)
    {
        ...
        if ($auth_check) {
            ...
            // Get notification preferences (for notification only, not blocking messages)
            ...
            \Apollo_Core\Email_Service::instance()->send(
                array(
                    'to'        => $receiver_user->user_email,
                    'subject'   => 'New Message from ' . $sender_first,
                    'body_html' => $receiver_body,
                    'flow'      => 'user_message_received',
                )
            );
            ...
            \Apollo_Core\Email_Service::instance()->send(
                array(
                    'to'        => $sender_user->user_email,
                    'subject'   => 'Your Message to ' . $receiver_first,
                    'body_html' => $sender_body,
                    'flow'      => 'user_message_sent',
                )
            );
    ...
    public function aprio_get_compatibilidade_conversation_list($request)
    {
        ...
        if ($auth_check) {
            ...
            foreach ($paginated_ids as $partner_id) {
                ...
                // Get profile photo from user meta with fallback
                $photo = get_user_meta($partner_id, '_profile_photo', true);
                if (empty($photo)) {
                    $photo = plugins_url('apollo-events-manager/assets/images/user-profile-photo.png');
                }
                $results[] = [
