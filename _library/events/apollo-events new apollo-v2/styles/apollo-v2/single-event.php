 ...
 *
 * LOCATION (CPT: "loc") — Connected via _event_loc_id:
 *   →  loc.post_title           → Location name in hero + sidebar
 *   →  _loc_address             → Full street address
 *   →  _loc_city                → City name
 *   →  _loc_lat / _loc_lng      → OpenStreetMap embed + Google Maps directions
...
$sounds      = wp_get_post_terms( $post_id, APOLLO_EVENT_TAX_SOUND, [ 'fields' => 'names' ] );
$categories  = wp_get_post_terms( $post_id, APOLLO_EVENT_TAX_CATEGORY, [ 'fields' => 'names' ] );
$types       = wp_get_post_terms( $post_id, APOLLO_EVENT_TAX_TYPE, [ 'fields' => 'names' ] );
...
wp_enqueue_style( 'apollo-events-v2', $style_url, [ 'apollo-events' ], APOLLO_EVENT_VERSION );
...
<div class="av2-event<?php echo esc_attr( $gone_class ); ?>"
        ...
        <section class="av2-hero">
            ...
            <div class="av2-hero__info">
                ...
                <div class="av2-info-block">
                    ...
                    <span class="av2-info-value">
                        <a href="<?php echo esc_url( $loc['permalink'] ?? '#' ); ?>"><?php echo esc_html( $loc['title'] ); ?></a><?php
                        if ( ! empty( $loc['city'] ) ) echo ', ' . esc_html( $loc['city'] );
                        ?>
                    </span>
                </div>
                <?php endif; ?>
    ...
    <div class="av2-container av2-layout">
        ...
        <aside class="av2-sidebar">
            ...
            <!-- ─── Location Mini ───────────────────────────────────
                 Source: _event_loc_id → loc CPT
                 Title:    loc.post_title
                 Address:  _loc_address
                 Map:      OpenStreetMap via _loc_lat + _loc_lng
                 Directions: Google Maps form with _loc_lat/_loc_lng
    ...
    $resale_args = [
        ...
        'posts_per_page' => 6,
        'meta_query'     => [
            [ 'key' => '_classified_event_id', 'value' => $post_id, 'type' => 'NUMERIC' ],
        ],
    ];
    ...
    <div class="av2-container">
        <div class="a-eve-single__section a-eve-resale">
            ...
            <div class="a-eve-resale__grid">
                <?php while ( $resale_query->have_posts() ) : $resale_query->the_post();
                    $r_id    = get_the_ID();
                    ...
                    $r_avatar = get_avatar_url( get_the_author_meta( 'ID' ), [ 'size' => 80 ] );
                    $r_date  = human_time_diff( get_the_time( 'U' ), current_time( 'timestamp' ) );
                ?>
                <a href="<?php the_permalink(); ?>" class="a-eve-resale__card">
                    <div class="a-eve-resale__card-top">
                        ...
                        <div class="a-eve-resale__seller">
                            ...
                            <span class="a-eve-resale__seller-time"><?php echo esc_html( $r_date . ' ' . __( 'atrás', 'apollo-events' ) ); ?></span>
                        </div>
                    </div>
        ...
        <div class="a-eve-single__section a-eve-depoimentos">
            ...
            <div class="a-eve-depo__timeline">
                <?php foreach ( $depoimentos as $idx => $depo ) :
                    $d_avatar = get_avatar_url( $depo->comment_author_email, [ 'size' => 80 ] );
                    $d_time   = human_time_diff( strtotime( $depo->comment_date ), current_time( 'timestamp' ) );
                ?>
                    ...
                    <div class="a-eve-depo__card">
                        <div class="a-eve-depo__card-header">
                            ...
                            <div class="a-eve-depo__meta">
                                ...
                                <span class="a-eve-depo__time"><?php echo esc_html( $d_time . ' ' . __( 'atrás', 'apollo-events' ) ); ?></span>
                            </div>
                        </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    tl.to('.av2-title-word', { y: '0%', duration: 1.2, stagger: 0.1, ease: 'power3.out' }, '-=0.5');

    // 3. Reveal hero meta & info
    tl.from('.av2-hero__meta, .av2-hero__info', { y: 40, opacity: 0, duration: 0.8, stagger: 0.1, ease: 'power2.out' }, '-=0.8');
    ...
    if (couponEl) {
        couponEl.addEventListener('click', function() {
            ...
            if (code && navigator.clipboard) {
                ...
                setTimeout(function() { couponEl.classList.remove('av2-coupon--copied'); }, 2000);
            }
        });
    }
});
