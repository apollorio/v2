...
class Shortcodes {
	...
	public function register(): void {
		// Shortcode principal
		add_shortcode( 'a-eve', [ $this, 'render_main' ] );

		// Shortcodes do registry
		add_shortcode( 'apollo_events', [ $this, 'render_apollo_events' ] );
		add_shortcode( 'apollo_event', [ $this, 'render_apollo_event' ] );
		add_shortcode( 'apollo_calendar', [ $this, 'render_apollo_calendar' ] );
		add_shortcode( 'apollo_event_form', [ $this, 'render_event_form' ] );

		// Registra no apollo-shortcodes (se ativo)
		add_filter( 'apollo_shortcodes_registry', [ $this, 'register_in_apollo_shortcodes' ] );
	}
	...
	public function render_apollo_event( $atts ): string {
		$atts = shortcode_atts( [ 'id' => 0 ], $atts, 'apollo_event' );
	...
	public function register_in_apollo_shortcodes( array $shortcodes ): array {
		$shortcodes['a-eve'] = [
			...
			'attrs'       => [ 'type', 'event', 'style', 'show', 'season', 'start', 'limit' ],
		];
		...
		if ( ! empty( $atts['season'] ) ) {
			$args['tax_query'] = [
				[
					...
					'field'    => 'slug',
					'terms'    => sanitize_text_field( $atts['season'] ),
				],
			];
		}
