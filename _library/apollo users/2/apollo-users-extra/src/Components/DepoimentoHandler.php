...
class DepoimentoHandler {
	...
	public function ajax_submit(): void {
		...
		if ( $author_id === $target_id ) {
			wp_send_json_error( [ 'message' => 'Não é possível escrever depoimento para si mesmo.' ] );
		}
		if ( empty( $text ) || strlen( $text ) < 10 ) {
			wp_send_json_error( [ 'message' => 'Mínimo de 10 caracteres.' ] );
		}
		if ( strlen( $text ) > 500 ) {
			wp_send_json_error( [ 'message' => 'Máximo de 500 caracteres.' ] );
		}
		...
		if ( $existing > 0 ) {
			wp_send_json_error( [ 'message' => 'Você já escreveu um depoimento para este usuário.' ] );
		}
		...
		wp_send_json_success( [ 'html' => $html, 'id' => $comment_id ] );
	}
