...
class ProfileController {
	...
	public function register_routes(): void {
		// Upload avatar
		register_rest_route( $this->namespace, '/profile/avatar', [
		...
		// Upload cover
		register_rest_route( $this->namespace, '/profile/cover', [
			'methods'             => \WP_REST_Server::CREATABLE,
			'callback'            => [ $this, 'upload_cover' ],
			'permission_callback' => 'is_user_logged_in',
		] );
		...
		register_rest_route( $this->namespace, '/profile/cover', [
			'methods'             => \WP_REST_Server::DELETABLE,
			'callback'            => [ $this, 'delete_cover' ],
			'permission_callback' => 'is_user_logged_in',
		] );

		// Get profile views (who viewed me)
		register_rest_route( $this->namespace, '/profile/views', [
			'methods'             => \WP_REST_Server::READABLE,
			'callback'            => [ $this, 'get_profile_views' ],
			'permission_callback' => 'is_user_logged_in',
			'args'                => [
				'limit' => [
					'default'           => 10,
	...
	public function get_profile_views( \WP_REST_Request $request ): \WP_REST_Response {
		...
		return rest_ensure_response( [ 'views' => $data ] );
	}
	...
	public function create_match( \WP_REST_Request $request ) {
		...
		$is_match = false;
		if ( $mutual && in_array( $action, [ 'like', 'superlike' ], true ) ) {
			$is_match = true;
	...
	public function get_matches( \WP_REST_Request $request ): \WP_REST_Response {
		...
		return rest_ensure_response( [ 'matches' => $data ] );
	}
}
