...
<section class="section-classifieds">
	...
	<div class="classified-row">
		...
		<div class="class-col">
			...
			<?php foreach ( $classifieds_hosting as $cl ) :
				$price = get_post_meta( $cl->ID, '_apollo_classified_price', true );
				$meta  = get_post_meta( $cl->ID, '_apollo_classified_subtitle', true );
				$icon  = get_post_meta( $cl->ID, '_apollo_classified_icon', true ) ?: 'ri-hotel-bed-line';
			?>
		...
		<div class="class-col">
			...
			<?php foreach ( $classifieds_tickets as $cl ) :
				$price = get_post_meta( $cl->ID, '_apollo_classified_price', true );
				$meta  = get_post_meta( $cl->ID, '_apollo_classified_subtitle', true );
			?>
			<a href="<?php echo esc_url( get_permalink( $cl ) ); ?>" class="class-item">
