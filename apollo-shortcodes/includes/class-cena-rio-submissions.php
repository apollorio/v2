...
class Cena_Rio_Submissions {
	...
	public static function rest_get_events( WP_REST_Request $request ): WP_REST_Response {
		...
		foreach ( $query->posts as $post ) {
			...
			$events[] = array(
				...
				'venue'                          => get_post_meta( $post->ID, '_event_local_name', true ),
	...
	public static function rest_submit_event( WP_REST_Request $request ) {
		$event_data = array(
			...
			'venue'       => $request->get_param( 'event_local' ) ?? '',
	...
	public static function handle_submission(): void {
		...
		$event_data = array(
			...
			'venue'       => isset( $_POST['event_local'] ) ? sanitize_text_field( wp_unslash( $_POST['event_local'] ) ) : '',
