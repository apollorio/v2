...
/**
 * Interesse Ranking functionality for Apollo Shortcodes.
 *
...
/**
 * Handles interesse ranking and related AJAX/shortcode functionality.
 */
final class Interesse_Ranking {

	private const TAXONOMIES          = array( 'event_sounds', 'event_listing_category', 'event_listing_type', 'event_listing_tag' );
	...
	public static function ajax_user_sounds(): void {
		...
		wp_send_json_success( array( 'sounds' => self::get_user_top_sounds( $uid, min( 20, absint( $_GET['limit'] ?? 10 ) ) ) ) );
	}
	...
	public static function shortcode_top_sounds( $atts ): string {
		...
		if ( empty( $sounds ) ) {
			return '<p class="apollo-empty">' . esc_html__( 'Nenhum som encontrado baseado nos seus interesses.', 'apollo-core' ) . '</p>';
		}
...
add_action( 'init', array( 'Apollo_Core\\Interesse_Ranking', 'init' ) );
