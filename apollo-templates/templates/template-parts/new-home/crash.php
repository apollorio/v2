...
<section class="section" id="crash" aria-labelledby="crash-title">
	<div class="container">
		<div class="nh-section-head ai">
			...
			<a href="<?php echo esc_url( home_url( '/acomoda' ) ); ?>" aria-label="<?php esc_attr_e( 'Ver todos os espaços', 'apollo-templates' ); ?>">Ver Todos →</a>
		</div>
		...
		<div class="nh-crash-grid">
			...
			<?php if ( $crash_query->have_posts() ) : ?>
				...
				while ( $crash_query->have_posts() ) :
					...
					if ( $card_count === 4 ) :
						?>
						<a href="<?php echo esc_url( $is_logged_in ? home_url( '/criar-anuncio' ) : $login_url ); ?>" class="nh-crash-card nh-crash-card--cta ai" aria-label="<?php esc_attr_e( 'Anuncie seu espaço', 'apollo-templates' ); ?>" translate="no">
							<span class="nh-cta-title" translate="no">Host Your Spot</span>
							<span class="nh-cta-sub" translate="no"><?php esc_html_e( 'Anuncie seu espaço', 'apollo-templates' ); ?></span>
							<span class="nh-cta-ptbr" translate="no">Join the network</span>
						</a>
					<?php endif; ?>

					<a href="<?php echo esc_url( $permalink ); ?>" class="nh-crash-card ai">
						<div class="nh-crash-img-wrap">
							<img src="<?php echo esc_url( $image ); ?>" class="nh-crash-img" alt="<?php echo esc_attr( get_the_title() ); ?>" loading="lazy" />
							<?php if ( ! $is_logged_in ) : ?>
